...
class Communicator {
    let apiKey = "RGAPI-ce83d7ba-099e-4bd8-bb0c-db3362163821"
    ...
    func getCallForSummunor(region: String, encodedName: String, completionHandler: @escaping ([String:Any]?, Error?) -> ()) {
        Alamofire.request("https://\(region).api.riotgames.com/lol/summoner/v4/summoners/by-name/\(encodedName)?api_key=" + apiKey).validate().responseJSON { response in
            switch response.result {
            case .success(let value):
    ...
    func getCallForSummunorMatches(_ region: String,_ accountId: String,_ completionHandler: @escaping ([[String:Any]]?, Error?) -> ()) {
        Alamofire.request("https://\(region).api.riotgames.com/lol/match/v4/matchlists/by-account/\(accountId)?beginIndex=0&endIndex=\(endIndexForGetMatchesCall)&api_key=" + apiKey).validate().responseJSON { response in
            switch response.result {
            case .success(let value):
    ...
    func getCallForMatch(_ region: String,_ matchId: String,_ completionHandler: @escaping ([String:Any]?, Error?) -> ()) {
        Alamofire.request("https://\(region).api.riotgames.com/lol/match/v4/matches/\(matchId)?api_key=" + apiKey).validate().responseJSON { response in
            switch response.result {
            case .success(let value):
                completionHandler((value as! [String:Any]), nil)
            case .failure(let error):
